#!/usr/bin/env python3
"""
encoder_proj_llm_finetune_ar_usage.py - PointLLM Encoder凍結 + Projector + LLM Full Fine-tuning

このスクリプトは事前学習されたEncoderの重みをロードし、Encoderを凍結した上で
Projection層とLLM全体をファインチューニングします。

使用方法:
    python encoder_proj_llm_finetune_ar_usage.py --gpu 0 --pretrained-path /path/to/encoder_projector_weights
"""
...
def load_config(config_path: str = None):
    ...
    if config_path and os.path.exists(config_path):
        ...
        config.training.gradient_accumulation_steps = 8  # 実質バッチサイズ64
        config.training.learning_rate = 5e-5  # LLM学習用に低めに設定
        config.output.use_wandb = False
        config.output.experiment_name = "ar_usage_encoder_proj_llm_finetune_v1"
