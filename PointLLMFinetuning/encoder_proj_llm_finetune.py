#!/usr/bin/env python3
"""
encoder_proj_llm_finetune.py - PointLLM Encoder凍結 + Projector + LLM Full Fine-tuning

このスクリプトは事前学習されたEncoderの重みをロードし、Encoderを凍結した上で
Projection層とLLM全体をファインチューニングします。

使用方法:
    python encoder_proj_llm_finetune.py --gpu 0 --pretrained-path /path/to/encoder_projector_weights
"""
...
# Blackwell Tensor Coreを有効化 (TF32)
...
def load_config(config_path: str = None):
    ...
    if config_path and os.path.exists(config_path):
        ...
        print("Using default config for Projection + LLM training")
        config = create_default_config()
        
        # Projection + LLM学習用のデフォルト設定
        config.model.freeze_llm = False  # LLMも学習する
        config.model.freeze_point_encoder = True  # Encoderは凍結
        config.model.use_flash_attention = True
        config.training.num_epochs = 10
        config.training.batch_size = 32  # メモリ使用量を考慮して削減
        config.training.gradient_accumulation_steps = 4  # 実質バッチサイズ64
        config.training.learning_rate = 1e-5  # LLM学習用に低めに設定
        config.output.use_wandb = False
        config.output.experiment_name = "demo_ar_low_lr_dataset_32_final_encoder_proj_llm_finetune_v8"
...
def prepare_model_and_tokenizer(config, pointllm_path: str, device: str, pretrained_path: str = None):
    """モデルとトークナイザーの準備"""
    print("\n" + "="*60)
    print("Loading PointLLM Model (Full LLM Training)")
    print("="*60)
    ...
    # Encoderを凍結（事前学習済みの重みを保持）
    for name, param in model.named_parameters():
        if "point_backbone" in name:
            param.requires_grad = False
    
    # Projection層とLLMを学習対象に設定
    for name, param in model.named_parameters():
        if "point_proj" in name or "llama_model" in name or "lm_head" in name:
    ...
    llm_trainable = sum(p.numel() for n, p in model.named_parameters() 
                       if p.requires_grad and ('llama_model' in n or 'lm_head' in n))
    ...
    print("✅ Projector and LLM are now trainable")
...
def run_training(model, tokenizer, train_loader, val_loader, config):
    """学習の実行"""
    print("\n" + "="*60)
    print("Starting Training (Projector + Full LLM)")
    print("="*60)
    
    if hasattr(torch, 'compile'):
        print("🚀 Compiling model with torch.compile for Blackwell...")
        # point_backboneは凍結されているため、学習対象のProjectorとLLMを重点的に最適化
...
def main():
    parser = argparse.ArgumentParser(
        description="PointLLM Encoder凍結 + Projector + LLM Full Fine-tuning"
    )
    parser.add_argument(
        "--gpu",
        type=int,
    ...
    print("="*60)
    print("PointLLM Encoder凍結 + Projector + LLM Full Fine-tuning")
    print("="*60)
