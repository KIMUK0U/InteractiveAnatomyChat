...
def init_model(args):
    ...
    # Refactored for device agnosticism
    device = get_device(args.device if hasattr(args, 'device') else None, verbose=True)

    # Determine dtype based on device compatibility
    # bfloat16 may not be fully supported on CPU/MPS, use float32 as fallback
    if device.type == 'cpu':
        dtype = torch.float32
        print(f'[INFO] Using float32 on CPU (bfloat16 not optimal for CPU)')
    elif device.type == 'mps':
        # MPS supports float32 and float16, but bfloat16 support may be limited
        dtype = torch.float16
        print(f'[INFO] Using float32 on MPS (bfloat16 support may be limited)')
    else:
        # CUDA supports bfloat16
        dtype = torch.bfloat16
        print(f'[INFO] Using bfloat16 on CUDA')
...
def start_generation(model, tokenizer, conv, dataloader, annos, prompt_index, output_dir, output_file):
    ...
    # Refactored for device agnosticism: use model's device
    model_device = next(model.parameters()).device
    input_ids_ = torch.as_tensor(inputs.input_ids).to(model_device) # * tensor of 1, L
    ...
    for batch in tqdm(dataloader):
        # Refactored for device agnosticism
        point_clouds = batch["point_clouds"].to(model_device).to(model.dtype) # * tensor of B, N, C(3)
...
def main(args):
    ...
    if not os.path.exists(args.output_file_path):
        ...
        # * release model and tokenizer, and release device memory
        del model
        del tokenizer
        # Clear cache for CUDA if available
...
if __name__ == "__main__":
    ...
    parser.add_argument("--device", type=str, default=None,
        ...
        help="Device to use for inference. If None, automatically selects best available device.")

    # * dataset type
    parser.add_argument("--data_path", type=str, default="pointllm/data/objaverse_data", required=False)
    parser.add_argument("--anno_path", type=str, default="pointllm/data/anno_data/PointLLM_brief_description_val_200_GT.json", required=False)
