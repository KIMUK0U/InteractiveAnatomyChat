...
def init_model(args):
    ...
    model = PointLLMLlamaForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=False, use_cache=True, torch_dtype=args.torch_dtype)
...
def start_conversation(args, model, tokenizer, point_backbone_config, keywords, mm_use_point_start_end, conv):
    ...
    while True:
        ...
        point_clouds = point_clouds.to(model_device).to(args.torch_dtype)
