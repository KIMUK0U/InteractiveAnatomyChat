...
def init_model(args):
    ...
    if device.type == 'cpu':
        dtype = torch.float16
        print(f'[INFO] Using float16 on CPU')
    elif device.type == 'mps':
        dtype = torch.float32
        print(f'[INFO] Using float32 on MPS')
    else:
        dtype = torch.float32  # Default to float32 for interactive use
        print(f'[INFO] Using float32 on CUDA')
...
def start_conversation(args, model, tokenizer, point_backbone_config, keywords, mm_use_point_start_end, conv):
    ...
    while True:
        ...
        def confirm_point_cloud(input_choice, object_id_input, point_cloud_input, chatbot, answer_time, conv):
            ...
            colors = color_data.astype(np.float32) / 255.0  # model input is (0-1)
            ...
            # point_clouds = torch.from_numpy(point_clouds).unsqueeze_(0).to(torch.float32).to(model_device)
            point_clouds = torch.from_numpy(point_clouds).unsqueeze_(0).to(torch.float32).to(model_device)
        ...
        def answer_generate(history, answer_time, point_clouds, conv):
            if point_clouds is None:
                ...
                # Refactored for device agnosticism: use model's device
                model_device = next(model.parameters()).device
                input_ids = torch.as_tensor(inputs.input_ids).to(model_device)
