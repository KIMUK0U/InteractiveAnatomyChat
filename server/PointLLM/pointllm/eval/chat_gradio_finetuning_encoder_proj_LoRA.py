...
def load_model_and_checkpoint(
    ...
    # ============================================================
    # Stage 1: Encoder + Projector の事前学習済み重みをロード
    # ============================================================
    if checkpoint_path:
        ...
        if os.path.exists(encoder_proj_path):
            ...
            print(f"   Path: {encoder_proj_path}")
            ...
            for ckpt_key, ckpt_value in state_dict.items():
                ...
                if ckpt_key.startswith("model."):
                    clean_key = ckpt_key[6:]  # "model." を削除
                else:
                    clean_key = ckpt_key
            ...
            print(f"\n   ✅ Loaded from: encoder_projector.pt")
            print(f"   - Total loaded parameters: {len(loaded_keys)}")
            print(f"   - Encoder parameters: {encoder_params}")
            print(f"   - Projector parameters: {projector_params}")
            print(f"   - Missing keys: {len(state_dict) - len(loaded_keys)}")
    
    # ============================================================
    # Stage 2: LoRA Adapter をロード
    # ============================================================
    ...
    # ============================================================
    # Stage 3: LoRAファインチューニング後のProjector更新をロード
    # ============================================================
    if checkpoint_path:
        point_proj_path = os.path.join(checkpoint_path, "point_proj.pt")
        
        if os.path.exists(point_proj_path):
            print(f"\n[Stage 3] Loading updated Projector weights...")
            print(f"   Path: {point_proj_path}")
            ...
            for ckpt_key, ckpt_value in state_dict.items():
                ...
                candidates = [
                    ckpt_key,
                    f"base_model.model.{ckpt_key}",
                    f"base_model.{ckpt_key}",
                ]
                
                # "model." を削除したバージョンも試す
                if ckpt_key.startswith("model."):
                    clean_key = ckpt_key[6:]
                    candidates.extend([
                        clean_key,
                        f"base_model.model.model.{clean_key}",
            ...
            if len(updated_keys) == 0:
                ...
                print(f"   🔍 Model keys (point_proj): {[k for k in model_state_dict.keys() if 'point_proj' in k][:5]}")
...
def generate_response(
    ...
    if device.type == 'mps':
        ...
        print(f"📊 [Inference Start] MPS Allocated: {torch.mps.driver_allocated_memory() / 1024**2:.2f} MB")

    # 2. 点群の転送とエンコード準備
    point_cloud = point_cloud.to(device=device, dtype=dtype)
    
    # --- [監視2] 点群をデバイスに乗せた直後のメモリ ---
    if device.type == 'mps':
        torch.mps.synchronize()
        print(f"📊 [Point to Device] MPS Allocated: {torch.mps.driver_allocated_memory() / 1024**2:.2f} MB")

    with torch.inference_mode():
        # model.generate 内部で Point-BERT が走り、その後に言語生成が始まる
    ...
    # --- [監視3] 生成完了（KVキャッシュが最大）の状態 ---
    if device.type == 'mps':
        torch.mps.synchronize()
        print(f"📊 [Generation End] MPS Allocated: {torch.mps.driver_allocated_memory() / 1024**2:.2f} MB")
