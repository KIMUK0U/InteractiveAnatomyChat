...
def seprate_point_cloud(xyz, num_points, crop, fixed_points = None, padding_zeros = False):
    ...
    for points in xyz:
        ...
        # Refactored for device agnosticism: use same device as input points
        if fixed_points is None:
            center = F.normalize(torch.randn(1,1,3),p=2,dim=-1).to(points.device)
        else:
            if isinstance(fixed_points,list):
                fixed_point = random.sample(fixed_points,1)[0]
            else:
                fixed_point = fixed_points
            center = fixed_point.reshape(1,1,3).to(points.device)
...
def random_scale(partial, scale_range=[0.8, 1.2]):
    # Refactored for device agnosticism: use same device as input tensor
    scale = torch.rand(1).to(partial.device) * (scale_range[1] - scale_range[0]) + scale_range[0]
